...
class Aliyun
{
    ...
    public static function send($filePath)
    {
        $url = '';
        $key = urlencode(basename($filePath));
        $bucketName = DI::getDefault()->get('config')->aliOss->PublicBucket;
        $ret = self::publicUpload($bucketName, $key, $filePath);
        if ($ret) {
            $url = self::getPublicObjectURL($bucketName, $key);

            if (empty($url)) {
                return DI::getDefault()->get('config')->aliyun_oss->cdn . $key;
            } else {
                return str_replace('http', 'https', $url);
            }
        } else {
            throw new Exception('图片上传阿里云失败', -1);
        }
    }
